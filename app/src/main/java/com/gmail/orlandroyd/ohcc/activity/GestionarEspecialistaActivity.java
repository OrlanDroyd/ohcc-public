...
public class GestionarEspecialistaActivity extends AppCompatActivity implements View.OnClickListener {
    private ProgressDialog pDialog;

    private TextInputEditText etName, etLastName1, etLastName2, etUserName, etPassword1, etPassword2, etCategoria;
    private TextInputLayout input_name, input_lastName1, input_lastName2, input_userName, input_password, input_password2, input_Categoria;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_especialista_gestionar);
    ...
    private void updateView() {
        ...
        Call<EspecialistaResponse> call = RetrofitClient.getInstance().getApi().getEspecialistaByID(ID);
        call.enqueue(new Callback<EspecialistaResponse>() {
            @Override
    ...
    private void verifyData() {
        ...
        if (!inputValidations.isInputEditTextFilled(etName, input_name, getString(R.string.empty))) {
            ...
            return;
        } else if (!inputValidations.isInputEditTextFilled(etCategoria, input_Categoria, getString(R.string.empty))) {
        ...
        if (!inputValidations.isInputEditTextName(etLastName2, input_lastName2, getString(R.string.name_invalido))) {
            ...
            if (getIntent().hasExtra(EXTRA_ID)) {
                ...
                if (id != -1) {
                    updateData(id, name, lastName1, lastName2, userName, password, categoria);
                }
            } else {
                submitData(name, lastName1, lastName2, userName, password, categoria);
            }

        }
    }

    private void updateData(int id, String name, String lastName1, String lastName2, String userName, String password, String categoria) {
        ...
        Call<DefaultResponse> call = RetrofitClient.getInstance().getApi().updateEspecialistaByDirectivo(id, name, lastName1, lastName2, userName, passwordEncrypted, categoria);
    ...
    private void submitData(String name, String lastName1, String lastName2, String userName, String password, String categoria) {
        showDialog(getString(R.string.enviando));
        String passwordEncrypted = "";
        try {
            // encrypt password
        ...
        Call<UsuarioEspecialistaCrearResponse> call = RetrofitClient.getInstance().getApi().crearEspecialistaByDirectivo(name, lastName1, lastName2, userName, passwordEncrypted, categoria);
        call.enqueue(new Callback<UsuarioEspecialistaCrearResponse>() {
            @Override
            public void onResponse(Call<UsuarioEspecialistaCrearResponse> call, Response<UsuarioEspecialistaCrearResponse> response) {
                hideDialog();
                if (response.isSuccessful()) {
                    if (response.code() == 201) {
                        FancyToast.makeText(getApplicationContext(), "Especialista " + getString(R.string.code_201), FancyToast.LENGTH_SHORT, FancyToast.SUCCESS, false).show();
                        Intent intent = new Intent();
                        setResult(RESULT_OK, intent);
                        finish();
                    } else if (response.code() == 421) {
            ...
            @Override
            public void onFailure(Call<UsuarioEspecialistaCrearResponse> call, Throwable t) {
                Log.e("GestEsp", "onFailure: ", t);
                hideDialog();
                FancyToast.makeText(getApplicationContext(), getString(R.string.code_422), FancyToast.LENGTH_SHORT, FancyToast.ERROR, false).show();
            }
    ...
    private void showDialogConfirm() {
        new AlertDialog.Builder(GestionarEspecialistaActivity.this)
    ...
    private void delete() {
        ...
        Call<DefaultResponse> call = RetrofitClient.getInstance().getApi().deleteEspecialista(ID);
        call.enqueue(new Callback<DefaultResponse>() {
            @Override
            public void onResponse(Call<DefaultResponse> call, Response<DefaultResponse> response) {
                hideDialog();
                if (response.isSuccessful()) {
                    if (response.code() == 200) {
                        FancyToast.makeText(getApplicationContext(), "Especialista " + getString(R.string.eliminado), FancyToast.LENGTH_SHORT, FancyToast.SUCCESS, false).show();
                        Intent intent = new Intent();
                        int id = getIntent().getIntExtra(EXTRA_ID, -1);
                        if (id != -1) {
                            intent.putExtra(EXTRA_ID, id);
                    ...
                    FancyToast.makeText(getApplicationContext(), "No puede eliminar esta cuenta porque tiene relación con los trámites de los usuarios", FancyToast.LENGTH_SHORT, FancyToast.ERROR, false).show();
                }
            }

            @Override
            public void onFailure(Call<DefaultResponse> call, Throwable t) {
                Log.e("GestEsp", "submitDatalure: ", t);
                hideDialog();
                FancyToast.makeText(getApplicationContext(), getString(R.string.code_422), FancyToast.LENGTH_SHORT, FancyToast.ERROR, false).show();
            }
        });
